class ComposerPhp72AT25 < Formula
  desc "Dependency Manager for PHP - Version 2.5.x"
  ...
  depends_on "sjorek/php/composer@25"

  def install

    php_binary      = "#{HOMEBREW_PREFIX}/opt/php@7.2/bin/php"
    composer_php    = "#{buildpath}/#{name}.php"
    composer_phar   = "#{HOMEBREW_PREFIX}/opt/composer@25/lib/composer.phar"
    ...
    inreplace composer_php do |s|
      ...
      composer_stub = <<~EOS
        ...
        if (false === getenv('COMPOSER_HOME') && !isset($_SERVER['COMPOSER_HOME'], $_ENV['COMPOSER_HOME'])) {
            putenv('COMPOSER_HOME=' . ($_SERVER['COMPOSER_HOME'] = $_ENV['COMPOSER_HOME'] = $_SERVER['HOME'] . '/.composer/composer25-php72'));
        }
  ...
  def caveats
    s = <<~EOS
      ...
      COMPOSER_HOME=${HOME}/.composer/composer25-php72
    EOS
