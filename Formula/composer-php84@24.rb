class ComposerPhp84AT24 < Formula
  ...
  def install
    ...
    php_binary      = "#{HOMEBREW_PREFIX}/opt/php@8.4/bin/php"
    ...
    inreplace composer_php do |s|
      ...
      composer_stub = <<~EOS
        ...
        if (false === getenv('COMPOSER_HOME') && !isset($_SERVER['COMPOSER_HOME'], $_ENV['COMPOSER_HOME'])) {
            putenv('COMPOSER_HOME=' . ($_SERVER['COMPOSER_HOME'] = $_ENV['COMPOSER_HOME'] = $_SERVER['HOME'] . '/.composer/composer24-php84'));
        }
  ...
  test do
    ...
    assert_match /^HelloHomebrew from version #{Regexp.escape("8.4")}$/,
  ...
  def caveats
    s = <<~EOS
      ...
      COMPOSER_HOME=${HOME}/.composer/composer24-php84
    EOS
