class ComposerPhp84AT27 < Formula
  ...
  depends_on "sjorek/php/composer@27"

  def install

    php_binary      = "#{HOMEBREW_PREFIX}/opt/php@8.4/bin/php"
    composer_php    = "#{buildpath}/#{name}.php"
    composer_phar   = "#{HOMEBREW_PREFIX}/opt/composer@27/lib/composer.phar"
    composer_setup  = "#{HOMEBREW_PREFIX}/opt/composer@27/lib/composer-setup.php"
    ...
    fail "invalid checksum for composer.phar" unless "b6de5e65c199d80ba11897fbe1364e063e858d483f6a81a176c4d60f2b1d6347" == composer_phar_sha256
    ...
    inreplace composer_php do |s|
      ...
      composer_stub = <<~EOS
        ...
        if (false === getenv('COMPOSER_HOME') && !isset($_SERVER['COMPOSER_HOME'], $_ENV['COMPOSER_HOME'])) {
            putenv('COMPOSER_HOME=' . ($_SERVER['COMPOSER_HOME'] = $_ENV['COMPOSER_HOME'] = $_SERVER['HOME'] . '/.composer/composer27-php84'));
        }
  ...
  def caveats
    s = <<~EOS
      ...
      COMPOSER_HOME=${HOME}/.composer/composer27-php84
    EOS
