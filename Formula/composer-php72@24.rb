class ComposerPhp72AT24 < Formula
  desc "Dependency Manager for PHP - Version 2.4.x"
  ...
  depends_on "sjorek/php/composer@24"

  def install

    php_binary      = "#{HOMEBREW_PREFIX}/opt/php@7.2/bin/php"
    composer_php    = "#{buildpath}/#{name}.php"
    composer_phar   = "#{HOMEBREW_PREFIX}/opt/composer@24/lib/composer.phar"
    composer_setup  = "#{HOMEBREW_PREFIX}/opt/composer@24/lib/composer-setup.php"

    composer_setup_sha384 = `#{php_binary} -r 'echo hash_file("sha384", "#{composer_setup}");'`
    fail "invalid checksum for composer-installer" unless "c8b085408188070d5f52bcfe4ecfbee5f727afa458b2573b8eaaf77b3419b0bf2768dc67c86944da1544f06fa544fd47" == composer_setup_sha384
    ...
    fail "invalid checksum for composer.phar" unless "c252c2a2219956f88089ffc242b42c8cb9300a368fd3890d63940e4fc9652345" == composer_phar_sha256
    ...
    inreplace composer_php do |s|
      ...
      composer_stub = <<~EOS
        ...
        if (false === getenv('COMPOSER_HOME') && !isset($_SERVER['COMPOSER_HOME'], $_ENV['COMPOSER_HOME'])) {
            putenv('COMPOSER_HOME=' . ($_SERVER['COMPOSER_HOME'] = $_ENV['COMPOSER_HOME'] = $_SERVER['HOME'] . '/.composer/composer24-php72'));
        }
  ...
  test do
    ...
    assert_match /^HelloHomebrew from version #{Regexp.escape("7.2")}$/,
  ...
  def caveats
    s = <<~EOS
      ...
      COMPOSER_HOME=${HOME}/.composer/composer24-php72
    EOS
