class ComposerPhp72AT28 < Formula
  ...
  def install
    ...
    fail "invalid checksum for composer.phar" unless "f446ea719708bb85fcbf4ef18def5d0515f1f9b4d703f6d820c9c1656e10a2f2" == composer_phar_sha256
    ...
    inreplace composer_php do |s|
      ...
      composer_stub = <<~EOS
        ...
        if (false === getenv('COMPOSER_HOME') && !isset($_SERVER['COMPOSER_HOME'], $_ENV['COMPOSER_HOME'])) {
            putenv('COMPOSER_HOME=' . ($_SERVER['COMPOSER_HOME'] = $_ENV['COMPOSER_HOME'] = $_SERVER['HOME'] . '/.composer/composer28-php72'));
        }
  ...
  def caveats
    s = <<~EOS
      ...
      COMPOSER_HOME=${HOME}/.composer/composer28-php72
    EOS
