class ComposerAT25 < Formula
    ...
    inreplace composer_php do |s|
      composer_stub = <<~EOS
        ...
        if (false === getenv('COMPOSER_HOME') && !isset($_SERVER['COMPOSER_HOME'], $_ENV['COMPOSER_HOME'])) {
            putenv('COMPOSER_HOME=' . ($_SERVER['COMPOSER_HOME'] = $_ENV['COMPOSER_HOME'] = $_SERVER['HOME'] . '/.composer/composer25-php'));
        }
  ...
  def caveats
    ...
    s = <<~EOS
      ...
      one or all of the sjorek/php/composer25-php* formulae.
      ...
      COMPOSER_HOME=${HOME}/.composer/composer25-php
    EOS
