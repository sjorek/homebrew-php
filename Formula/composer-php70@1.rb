class ComposerPhp70AT1 < Formula
  ...
  def install
    ...
    php_binary      = "#{HOMEBREW_PREFIX}/opt/php@7.0/bin/php"
    ...
    fail "invalid checksum for composer.phar" unless "230d28fb29f3c6c07ab2382390bef313e36de17868b2bd23b2e070554cae23d2" == composer_phar_sha256
    ...
    inreplace composer_php do |s|
      ...
      composer_stub = <<~EOS
        ...
        if (false === getenv('COMPOSER_HOME') && !isset($_SERVER['COMPOSER_HOME'], $_ENV['COMPOSER_HOME'])) {
            putenv('COMPOSER_HOME=' . ($_SERVER['COMPOSER_HOME'] = $_ENV['COMPOSER_HOME'] = $_SERVER['HOME'] . '/.composer/composer1-php70'));
        }
  ...
  test do
    ...
    assert_match /^HelloHomebrew from version #{Regexp.escape("7.0")}$/,
  ...
  def caveats
    s = <<~EOS
      ...
      COMPOSER_HOME=${HOME}/.composer/composer1-php70
    EOS
